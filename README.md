...
│   └── stage2_5.job            #   Stats + charge + QC (aggregation)
│
...
This runs the full pipeline: adapter removal, barcode splitting, UMI trimming, SWIPE alignment, stats collection, charge quantification, and QC report generation.
...
| 2 | Stats collection + aggregation | `ALL_stats_aggregate.csv` |
| 3 | Charge quantification | `charge_analysis/charge_df_{aa,codon,transcript}.csv` |
| 5 | QC dashboard | `QC_report.html`, `QC_summary.csv` |
...
Stage 5 generates an interactive HTML report (`QC_report.html`) with 8 panels:
