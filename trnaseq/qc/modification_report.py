...
<div class="footer">Generated by trnaseq modification QC module</div>
    ...
    def _panel_modification_type_distribution(self):
        ...
        return (
            f'<h2>{heading}</h2>'
            f'<p style="color:#636e72;margin-top:-10px;font-size:13px;">'
            f'{subtitle}</p>'
    ...
    def _panel_modification_landscape(self):
        ...
        div = self._fig_to_div(fig)
        return (
            f'<h2>Modification Landscape</h2>'
            f'<p style="color:#636e72;margin-top:-10px;font-size:13px;">'
            f'Mean mismatch rate per tRNA and absolute position. Positions from all '
            f'tRNA lengths are mapped to the longest tRNA using nearest-percentile '
            f'interpolation (consistent with Behrens/Needle coverage plots). '
            f'Bright cells indicate positions with elevated substitution signal '
            f'(likely RNA modifications). Each row is one tRNA species; '
            f'x-axis spans 5&prime; to 3&prime; end.</p>'
            f'<div class="card">{div}</div>'
        )
    ...
    def _panel_call_reproducibility(self):
        ...
        div = self._fig_to_div(fig)
        return (
            f'<h2>Modification Call Reproducibility</h2>'
            f'<div class="card">{div}</div>'
        )
    ...
    def _panel_synthetic_false_positives(self):
        ...
        div = self._fig_to_div(fig)
        return (
            f'<h2>Synthetic Control Error Rate</h2>'
            f'<div class="card">{div}</div>'
        )
